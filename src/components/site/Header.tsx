...
export function Header() {
  ...
  return (
    ...
    >
      ...
      >
        <Link to="/" className="flex items-center gap-3" aria-label="MSL Colombo — home">
          <img
            src={logo.url}
        ...
        <nav className="hidden items-center gap-9 lg:flex" aria-label="Main">
          <Link to="/" className={navLink}>
            Home<span className="link-underline-bar" />
          </Link>
          <Link to="/about-us" className={navLink}>
            About Us<span className="link-underline-bar" />
          </Link>
          ...
          <div className="relative" onMouseEnter={hoverOpen} onMouseLeave={hoverClose}>
            ...
            >
              <div className="glass-panel overflow-hidden rounded-md bg-[color:var(--navy-deep)]/95 p-2">
                <p className="eyebrow px-3 pb-2 pt-2 text-on-navy-muted">Our Divisions</p>
                {services.map((s, i) => (
                  <Link
                    key={s.to}
                    to={s.to}
                  ...
                  >
                    <span className="flex items-center gap-3">
                      <span className="eyebrow text-on-navy-muted">{`0${i + 1}`}</span>
                      {s.label}
                    </span>
          ...
          <Link to="/contact-us" className={navLink}>
            Contact Us<span className="link-underline-bar" />
          </Link>
        </nav>
      ...
      >
        <nav className="px-5 py-4" aria-label="Mobile">
          <Link to="/" className="block border-b border-[color:var(--hairline-navy)] py-4 text-lg text-on-navy">
            Home
          </Link>
          <Link
            to="/about-us"
          ...
          <div className="border-b border-[color:var(--hairline-navy)]">
            ...
            <div
              className={cn(
                ...
                servicesOpen ? "max-h-72" : "max-h-0",
              )}
            >
              <div className="pb-3 pl-4">
                <Link to="/services" className="block py-2.5 text-on-navy-muted">
                  All Services
                </Link>
                {services.map((s) => (
                  <Link key={s.to} to={s.to} className="block py-2.5 text-on-navy-muted">
                    {s.label}
                  </Link>
                ))}
              </div>
